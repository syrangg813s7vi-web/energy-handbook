...
## 2026-09-11：间隔知识 Top-Down 文章

- 间隔是围绕一条母线连接支路形成的可独立管理功能单元，不等于断路器、开关柜或空间。
- 文章需同时表达一次设备、过程层、间隔层和站控层；保护动作本地自治，站控承担跨间隔协调，PPC连续功率控制不属于间隔闭环。
- 西门子产品映射以仓库既有官方资料为证据：SIPROTEC 5系列、6MD85、6MU85、SICAM PAS/S8000、SCC、A8000、DIGSI 5、SIGRA。SIPROTEC V按功能属于间隔保护控制，物理上运行在站级服务器。
- 首轮 SVG 视觉检查发现接地刀闸被画在一次功率串联路径上，容易误导；已改为从线路节点引出的对地支路，并在图注说明实际设备顺序随接线变化。
...
- 正式站点1440×900复验与本地结果一致：正文960px，三张图内部尺寸分别为958×852、958×648、958×848，逐元素扫描均没有可滚动容器。
