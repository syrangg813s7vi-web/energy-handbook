...
## 2026-09-22：RK3588 AI边缘计算盒文章

- 用户要求以表格发布三类信息：硬件配置、AI硬件能力与适用应用、厂家宣称支持的AI应用。
- 已有调研将截图机型高置信匹配到G6-RK3588-F13平台，但仍按OEM/具体SKU不确定处理。
- 文章最终放在行业洞察域：它同时包含硬件平台判断与厂家主张，比纯工业控制基础知识更符合现有信息架构；标题与路由保持产品中立。
- 关键结论是6 TOPS NPU适合轻量视觉推理，不等于八路满帧AI，也不等于基础SKU预装全部算法。
- 发布目标是GitHub Pages上的EnergyBook，而非Notion；权威源为Git仓库和现有VitePress构建流程。

## 2026-09-11：间隔文章状态主图改版

- 自评结论：上一版已降低首屏难度，但类比没有贯穿全文，静态简图只展示设备顺序，没有展示三种状态下谁变化；完整架构图和 Siemens 型号对初学者仍偏密。
- 新版必须复用同一套设备位置，只改变开关状态、电流/安全路径和解释，减少读者在三张不同图之间重新建立空间关系的负担。
- 完整四层架构图仍有价值，但应降级为“已经看懂主图后的进阶展开”，不再承担第一核心教学任务。
- “间隔每天只是在处理三件事”表述过度简化，改为“先用三种典型状态理解间隔”。
- 站点规范要求交互原版放在 `docs/public/demos/<topic>/`，Markdown 仅 iframe 嵌入；必须有键盘控件、`aria-live` 状态、无脚本基础信息、减弱动画支持、移动端不溢出和正文文字降级。
- 既有轻量演示采用原生 HTML + SVG + CSS + JavaScript，状态保存在根节点 `data-*` 属性中，适合本轮三状态切换；不需要引入框架或运行时依赖。
- 状态主图采用响应式 HTML 设备节点而非固定宽 SVG：桌面为同一条横向支路，手机自动变为纵向支路，因此无需在小屏缩小文字或引入图内横向滚动。
- 正文保留原任务要求的作用、周边通信、组件原理/必要性、分类和 Siemens 映射，但完整四层架构默认折叠为进阶内容，核心学习不再依赖密集图。
- 首次浏览器快照确认状态图的三个按钮、传感—保护—断路器链和正常状态说明均进入可访问性树；唯一控制台项来自无实际隔离收益的 `allow-scripts allow-same-origin` sandbox 组合，已移除该属性。
- 故障与检修截图确认断路器开断、保护高亮、状态文字和按钮选中态同步；同时发现接地支路初稿落在 CT/VT 下方，拓扑含义不准确，已把支路连接点移到断路器后的线路侧并补连续引线。
- 接地修正后实测连接点横坐标 731，位于断路器右缘 712 与线路节点起点 746 之间，拓扑位置正确；桌面 iframe 958×688 无内部溢出。手机端判断链仍过密并有 151 px 纵向溢出，继续改为全纵向布局。
- 手机判断链改为纵向后文字不再挤压；390 px 页面与 325 px iframe 均无横向溢出，三种状态可点击。内容实际底部为 1294 px，将 iframe 从临时 1420 px 收紧到 1310 px，保留约 14 px 余量且不产生内部滚动。
- 打开进阶区后页面滚动条件使状态图短暂出现纵向滚动条，客户宽从 325 px 缩到 310 px，换行又把内容推高到 1324 px；移动高度最终设为 1340 px，以覆盖这类最窄反馈循环。
- 最终移动复验为页面 375/375 px、iframe 325/325 px、iframe 高度 1338/1338 px，横纵方向均无内部溢出；检修按钮保持 `aria-pressed=true`，状态数据为 `maintenance`。桌面复验为页面 1425/1425 px、iframe 958/958×688/688 px，控制台无 warning。
- 第三版正文由 6703 字符降到 5204 字符，减少约 22%；减少部分主要来自静态简图、重复状态卡和分散的长段解释，而非删去原任务要求的知识模块。
- 从当前提交克隆到全新临时目录后，标准 `npm ci && npm run check` 成功：139 个依赖按锁文件安装，23 项测试和 VitePress 构建通过，证明状态图与文章可从 Git 权威输入恢复。
- 第三版经 PR #83 合入 `main`，合并提交 `57e07d590ea747a2f6a6c91722cdf946dfbe3d78`；Pages 运行 `34586441004` 的 build 与 deploy 成功。正式文章、状态图和进阶图均返回 HTTP 200。
- 正式站点 390×844 实际点击故障和检修成功；最终为页面宽 375/375、状态图 325/325×1338/1338，选中按钮“③ 检修”，结果文字“停电检修：断流、隔离、验电后接地”，控制台无 warning。

## 2026-09-11：间隔文章直观化改版

- 用户明确目标读者“不特别熟悉电站系统”，现版以四层架构和组件术语起步，认知负担过高。
- 新版先用“一条电能支路的一套门和安全员”建立直觉：母线是公共电源通道，断路器是可带故障电流分断的门，隔离开关提供可见隔离，接地开关把停电部分可靠接地，CT/VT 是传感器，保护与测控装置负责判断和执行。
- 对初学者最有效的主线不是设备清单，而是三种状态：正常时送电，故障时保护命令断路器切断，检修时先断开并隔离、验电后接地。产品型号应在概念理解之后出现。
- 完整关系图仍需保留，但作为第二层“放大看全貌”；首屏使用低密度电能路径和状态卡片，避免要求读者先解码复杂架构图。
- Archify 首版验证显示边界、端口方向与标签间距问题；这些属于图形可读性缺陷，修复后再进入正文嵌入。
- 关系图一次设备链上的箭头只用于引导读者从母线向外识别设备，不代表固定潮流方向；光伏集电线路通常向母线送电，储能还可能双向流动，正文图注必须明确这一点。
- 本地生产页面在 1440×1000 下正文扩为 960 px：简图 5 个节点、3 张状态卡和 958×718 的完整图全部加载，页面与图内均无横向溢出。390×844 下简图和状态卡转为单列，页面宽度、组件滚动宽度与客户区一致。
- 桌面首屏截图显示读者无需滚动即可看到一句话定义、完整简单路径和三状态卡片的开头；移动首屏标题、类比和定义完整，没有被复杂图抢占。
- 改版经 PR #81 合入 `main`，合并提交 `16ba5f39ccea3ff83de965404ac7cd49953b84ef`；Pages 运行 `34583595638` 的 build 与 deploy 均成功。正式文章、完整图和 sitemap 均为 HTTP 200，线上正文包含 5 个简图节点和 3 张状态卡，958 px 宽内嵌图无横向溢出。
...
# 西门子电站产品列表调研（2026-09-16）

- Siemens Energy 官方产品总览列出燃气轮机、蒸汽轮机、发电机、绿氢、电网设备；发电方案页覆盖简单循环、联合循环、供热、核电常规岛等。
- Siemens Gamesa 官方风电产品目录覆盖陆上和海上风机；具体机型与地区可售状态应分开表述。
- Siemens AG 的 SICAM PPC 属于光伏电站并网控制，不是光伏组件或逆变器。
- 当前 EnergyBook 主线已包含西门子变电站架构与资料库文章，可互链；以独立工作树避免干扰其他进行中的图文发布。
- 最终清单共 19 条，按发电主机与热力、新能源与储能、控制保护及升压并网分组；其中 17 个不同的官方入口均于 2026-09-16 返回 HTTP 200。

## 新能源整体方案范围修订（2026-09-16）

- 用户进一步明确想看的是新能源电站整体解决方案的产品矩阵，原广义电站清单中的燃气电站和常规热力系统不再是主体。
- 官方资料确认 SICAM Microgrid Control 用于微网内可再生电源、储能、负荷的协调；Omnivise Hybrid Control 基于 T3000，面向复杂混合场站，与 SCADA 和 EMS 配合。
- Siemens Gamesa 官方服务资料确认风机诊断和 SCADA 升级；Siemens Energy 官方海上风电控制页确认 T3000/Omnivise 在海上风场监控与混合运行中的角色。
- 矩阵应同时展示“功能层级 × 硬件/软件 × 典型场景 × 供货主体/第三方边界”，并用组合示例说明各层怎样连在一起。
