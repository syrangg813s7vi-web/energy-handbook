...
```text
柴油发动机 → 1号发电机G1 → 出口断路器QF1 → 母线 → 负载
```

- **柴油发动机**提供机械动力。
- **发电机G1**把机械能转换成电能，G1表示1号发电机。
