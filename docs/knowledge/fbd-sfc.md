...
```text
// 状态变量
X0, X1, X2, X3, X4 : BOOL;   // 等待、夹紧、灌装、封盖、放行
T0, T1, T2, T3, T4 : BOOL;   // 转换条件

// Network 0：首次扫描时进入等待状态
IF FirstScan THEN X0 := TRUE; END_IF;
...
// Network 2：退出当前状态，进入下一状态
IF T0 THEN X0 := FALSE; X1 := TRUE; END_IF;
IF T1 THEN X1 := FALSE; X2 := TRUE; END_IF;
IF T2 THEN X2 := FALSE; X3 := TRUE; END_IF;
IF T3 THEN X3 := FALSE; X4 := TRUE; END_IF;
IF T4 THEN X4 := FALSE; X0 := TRUE; END_IF;

// Network 3：根据状态计算设备输出
RunOK      := 自动模式 AND 安全允许;
...
每个PLC扫描周期都会先读取输入，然后按照程序确定的顺序执行全部FBD网络：先计算 `T0～T4`，再执行五组状态切换，最后重新计算全部输出。即使本周期只有 `T2=TRUE`，其他转换、状态切换和输出代码也会被逐行扫描。程序执行结束后，新的状态BOOL和输出值被保存，供下一个扫描周期继续使用。
...
STEP X3_封盖
    ACTION N: 夹具请求 := TRUE;
    ACTION N: 封盖机请求 := TRUE;
TRANSITION X3_封盖 → X4_放行 WHEN 封盖完成;

STEP X4_放行
    ACTION N: 输送带请求 := TRUE;
TRANSITION X4_放行 → X0_等待 WHEN 瓶离开;

// 所有设备输出仍经过公共安全联锁
物理输出 := SFC动作请求 AND 自动模式 AND 安全允许;
```
