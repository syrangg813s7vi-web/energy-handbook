...
5. Cloud Codex 修改 Markdown 或动画代码、运行构建，并生成可审查的任务差异。
6. `molt` 上的受限发布器应用差异，并用仅绑定本仓库的 Deploy Key 推送 `codex/review-*` 临时分支。
7. 仓库 Action 自动检查文件范围、安全规则和 VitePress 构建，随后创建并 squash 合并 PR；GitHub Pages 自动发布新版。
