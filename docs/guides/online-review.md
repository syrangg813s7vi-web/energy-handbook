...
登录由 Cloudflare Access 负责，不在知识库中维护密码。身份验证完成后，专用登录网关验证 Access JWT 的签名、签发者和应用受众，再返回只能使用一次、两分钟过期的授权码；网站用它换取 15 分钟批阅令牌。GitHub Token、ChatGPT Token 和服务密钥不会发送给浏览器。
