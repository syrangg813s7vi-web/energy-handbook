...
<template>
  <div class="config-demo sample-manager-demo">
    ...
    <div class="sample-stage">
      <svg viewBox="0 0 736 300" role="img" aria-labelledby="sample-title"><title id="sample-title">采样数据从设备流向实时缓存、历史库和消费者</title>
...
<style scoped>
.sample-stage{position:relative;min-height:300px;aspect-ratio:736/300}.sample-stage svg{position:absolute;inset:0;width:100%;height:100%}.wire{fill:none;stroke:var(--config-line);stroke-width:2}.wire.active{stroke:var(--config-accent)}.packet{r:5;fill:var(--config-accent)}.node{position:absolute;box-sizing:border-box;display:flex;flex-direction:column;justify-content:center;gap:4px;min-height:84px;padding:10px;border:1px solid var(--config-line);border-radius:10px;background:var(--config-node);transition:.4s}.node span,.node small{color:var(--vp-c-text-2)}.node.on{border-color:var(--config-accent);background:var(--config-accent-soft)}.source{top:103px;left:0;width:18.3%}.manager{top:103px;left:31.2%;width:19%}.cache{top:38px;left:60.5%;width:19%}.history{top:178px;left:60.5%;width:19%}.consumer{top:103px;right:0;width:14.3%}
@media(max-width:680px){.sample-stage{display:grid;grid-template-columns:1fr 1fr;gap:10px;min-height:0;aspect-ratio:auto}.sample-stage svg{display:none}.node{position:static;width:auto;min-height:92px}.source{order:1}.manager{order:2}.cache{order:3}.history{order:4}.consumer{order:5;grid-column:1/-1}}
